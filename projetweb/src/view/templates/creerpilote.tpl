...
<body>
    ...
    {if isset($smarty.session.id_membre) || isset($smarty.session.etudiant) || $smarty.session.etudiant || isset($smarty.session.pilote) || $smarty.session.pilote}
            {if (isset($smarty.session.admin) && $smarty.session.admin)}
                <h1>Creer un compte pilote</h1>
